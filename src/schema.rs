...
fn rank(v: &str) -> Option<u8> {
    if v.is_empty() { Some(0) } else { VERSIONS.iter().position(|version| *version == v).map(|i| i as u8) }
}
...
fn schema_index() -> &'static SchemaIndex {
    ...
    INDEX.get_or_init(|| {
        ...
        for (id, t) in schema()["types"].as_object().unwrap() {
            ...
            if !t["is_abstract"].as_bool().unwrap_or(false) {
                index.roots.entry(name).and_modify(|v| *v = None).or_insert(Some(id));
            }
            let children = index.children.entry(id).or_default();
            for child in arr(&t["children"]).iter().map(s) {
                children.entry(expanded(qname(child))).and_modify(|v| *v = None).or_insert(Some(child));
            }
            if !t["particle"].is_null() { index.particles.insert(id, Particle::compile(&t["particle"])); }
        }
        for (i, rule) in arr(&schema()["semantics"]).iter().enumerate() {
            index.semantics.entry(expanded(s(&rule["Context"]))).or_default().push((i, rule));
        }
        for definition in schema()["enums"].as_object().unwrap().values() {
            index.enums.insert(s(&definition["full_name"]), definition);
        }
        index
    })
}
...
pub fn element_type(xml: &Xml, id: usize) -> PyResult<Option<&'static str>> {
    ...
    let mut path = ancestors(doc, id).collect::<Vec<_>>(); path.reverse();
    let mut context = None;
    for node in path {
        if node.element().unwrap().name.uri != MC { context = resolve(node, context.as_deref()); }
    }
...
fn pattern(pattern: &str, value: &str, gaps: &mut BTreeSet<String>) -> bool {
    ...
    match regex { Some(regex) => regex.is_match(value), None => { gaps.insert(format!("pattern:{pattern}")); true } }
}

fn base64(value: &str) -> Option<Vec<u8>> {
    use base64::{engine::general_purpose::{GeneralPurpose, PAD}, Engine};
...
fn float(value: &str, single: bool) -> Option<f64> {
    match value { "INF" => Some(f64::INFINITY), "-INF" => Some(f64::NEG_INFINITY), "NaN" => Some(f64::NAN),
...
fn decimal(value: &str, gaps: &mut BTreeSet<String>) -> bool {
    ...
    if fraction.len() > 28 || format!("{whole}{fraction}").parse::<u128>().map_or(true, |v| v > MAX) {
        gaps.insert("decimal:precision-rounding".into());
    }
    true
}
...
fn datetime(value: &str, gaps: &mut BTreeSet<String>) -> bool {
    ...
    if !value.contains('T') && pattern(r"([0-9]{4}|[0-9:+.Z-]*[-:][0-9:+.Z-]*)",value,gaps) {
        ...
        gaps.insert("datetime:non-timestamp".into()); return true;
    }
    if !pattern(r"[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}(\.[0-9]+)?(Z|[+-][0-9]{2}:[0-9]{2})?",value,gaps) { return false; }
    let parsed = chrono::DateTime::parse_from_rfc3339(value).map(|t| t.naive_local()).or_else(|_|
...
fn boolean(kind: &str, value: &str) -> Option<bool> {
    let value = match kind {
        "BooleanValue" => value.trim_matches([' ', '\t', '\r', '\n']),
        "OnOffValue" => value,
        _ => return None,
    };
    match value {
        "true" | "1" => Some(true), "false" | "0" => Some(false),
        "on" if kind == "OnOffValue" => Some(true), "off" if kind == "OnOffValue" => Some(false),
        _ => None,
    }
}
...
fn lexical(kind: &str, value: &str, gaps: &mut BTreeSet<String>) -> bool {
    ...
    match kind {
        ...
        "Int16Value" => number.parse::<i16>().is_ok(), "Int32Value" => number.parse::<i32>().is_ok(),
        "Int64Value" | "IntegerValue" => number.parse::<i64>().is_ok(), "UInt16Value" => number.parse::<u16>().is_ok(),
        "UInt32Value" => number.parse::<u32>().is_ok(), "UInt64Value" => number.parse::<u64>().is_ok(),
        "ByteValue" => number.parse::<u8>().is_ok(), "SByteValue" => number.parse::<i8>().is_ok(),
        "DoubleValue" | "SingleValue" => float(number, kind == "SingleValue").is_some(),
        "DecimalValue" => decimal(number,gaps), "DateTimeValue" => datetime(number,gaps),
        ...
        _ if kind.starts_with("EnumValue<") => {
            ...
            match definition { Some(e) => {
                ...
                None => { gaps.insert(format!("enum-source:{name}")); true } }
        }
        _ => { gaps.insert(format!("lexical:{kind}")); true }
    }
}
...
fn validator(v: &Value, value: &str, kind: &str, gaps: &mut BTreeSet<String>) -> bool {
    ...
    let kind = if override_type.is_empty() { kind } else if let Some(kind) = schema()["simple_types"][override_type].as_str() { kind }
        else if name == "EnumValidator" {
            if let Some(e) = schema()["enums"].get(override_type) { return arr(&e["Facets"]).iter().any(|f| s(&f["Value"]) == value); }
            gaps.insert(format!("validator-type:{override_type}")); return true;
        } else { gaps.insert(format!("validator-type:{override_type}")); return true; };
    ...
    arr(&v["Arguments"]).iter().all(|a| {
        let key = s(&a["Name"]); let val = s(&a["Value"]);
        let bound = val.parse::<i128>().ok();
        let compare = |b: i128| if matches!(kind,"DoubleValue" | "SingleValue") {
            float(number,kind == "SingleValue").and_then(|n| n.partial_cmp(&(b as f64)))
        } else { number.parse::<i128>().ok().map(|n| n.cmp(&b)) };
        let length = if kind == "HexBinaryValue" { value.len() / 2 }
            else if kind == "Base64BinaryValue" { base64(value).map_or(0, |b| b.len()) }
            else { value.encode_utf16().count() } as i128;
        use std::cmp::Ordering::{Less, Equal, Greater};
        match (key, bound) {
            ("MinInclusive", Some(b)) => matches!(compare(b), Some(Equal | Greater)),
            ("MaxInclusive", Some(b)) => matches!(compare(b), Some(Equal | Less)),
            ("MinExclusive", Some(b)) => compare(b) == Some(Greater), ("MaxExclusive", Some(b)) => compare(b) == Some(Less),
            ...
            ("MinLength", Some(b)) => length >= b, ("MaxLength", Some(b)) => length <= b, ("Length", Some(b)) => length == b,
            ...
            ("IsQName", _) => val.eq_ignore_ascii_case("false") || {
                let names = value.split(':').collect::<Vec<_>>();
                names.len() <= 2 && names.iter().all(|n| crate::xml::check_local(n).is_ok())
            },
            ("IsToken", _) => val.eq_ignore_ascii_case("false") || !value.starts_with(' ') && !value.ends_with(' ') && !value.contains("  ") && !value.contains(['\t','\n','\r']),
            _ => { gaps.insert(format!("facet:{name}.{key}")); true }
        }
    })
}
...
fn check_value(kind: &str, validators: &[Value], value: &str, target: &str, gaps: &mut BTreeSet<String>) -> Vec<String> {
    ...
    for v in validators.iter().filter(|v| applicable(v,target)) {
        ...
        if let Some(id) = v["UnionId"].as_i64() { unions.entry(id).or_default().push((valid,local)); }
        else {
            gaps.extend(local);
    ...
    for branches in unions.values() {
        ...
        if branches.iter().any(|(valid,_)| *valid) {
            for (_,local) in branches { gaps.extend(local.iter().cloned()); }
        } else { errors.push("union".into()); }
    }
    errors
}
...
fn check_attr(a: &Value, value: Option<&str>, target: &str, gaps: &mut BTreeSet<String>) -> Vec<String> {
    ...
    let mut errors = Vec::new();
    if value.is_none() {
        if available(s(&a["Version"]),target) && validators.any(|v| s(&v["Name"]) == "RequiredValidator" &&
...
fn particle_occurs(p: &Value, target: &str) -> Option<(usize, usize)> {
    ...
    occurs.iter().filter(|o| version(o).is_some_and(|v| v <= target_rank)).max_by_key(|o| version(o))
        .map(|o| (o["Min"].as_u64().unwrap_or(0) as usize, o["Max"].as_u64().unwrap_or(0) as usize))
}

enum Term {
    Element(&'static str), Sequence(Vec<Particle>), Choice(Vec<Particle>),
    Any(&'static str), All(Vec<Particle>), Unsupported(&'static str),
}

struct Particle {
    occurs: [Option<(usize, usize)>; 7],
    require_filter: bool,
    term: Term,
}

fn positions(mut values: Vec<usize>) -> Vec<usize> {
    values.sort_unstable(); values.dedup(); values
}
...
impl Particle {
    fn compile(p: &'static Value) -> Self {
        ...
        let term = if let Some(name) = p["Name"].as_str() { Term::Element(name) } else {
            match s(&p["Kind"]) {
                "Sequence" | "Group" => Term::Sequence(items()), "Choice" => Term::Choice(items()),
                "All" => Term::All(items()), "Any" => Term::Any(s(&p["Namespace"])), other => Term::Unsupported(other),
            }
        };
        Self { occurs: std::array::from_fn(|i| particle_occurs(p,VERSIONS[i])),
            require_filter: p["RequireFilter"].as_bool().unwrap_or(false), term }
    }

    // Same bounded matcher, over immutable compiled descriptors and contiguous position sets.
    fn matches(&self, children: &[(&str, &str)], start: usize, parent_uri: &str, target: usize,
        gaps: &mut BTreeSet<String>, budget: &mut usize) -> Vec<usize> {
        if *budget == 0 { gaps.insert("particle:budget".into()); return Vec::new(); }
        *budget -= 1;
        ...
        let mut results = Vec::new(); let mut current = vec![start];
        ...
        for count in 1..=max {
            ...
            for &pos in &current {
                match &self.term {
                    ...
                    Term::Choice(items) => for item in items { next.extend(item.matches(children,pos,parent_uri,target,gaps,budget)); },
                    Term::Any(namespace) => if let Some((_, uri)) = children.get(pos) {
                        let matches = match *namespace {
                            "" | "##any" => true, "##local" => uri.is_empty(),
                            "##other" => uri.is_empty() || *uri != parent_uri, "##targetNamespace" => *uri == parent_uri,
                            other => { gaps.insert(format!("particle:namespace:{other}")); false }
                        };
                    ...
                    Term::All(items) => {
                        ...
                        for item in items {
                            ...
                            if let Term::Element(name) = item.term {
                                if min <= 1 && max == 1 { members.insert(name, min); continue; }
                            }
                        ...
                        for (offset, (id, _)) in children[pos..].iter().enumerate() {
                            if *budget == 0 { gaps.insert("particle:budget".into()); break; }
                            *budget -= 1;
                    ...
                    Term::Unsupported(kind) => { gaps.insert(format!("particle:{kind}")); }
                }
            }
...
fn ignorable(doc: &Document, id: usize, uri: &str) -> bool {
    ...
    ancestors(doc,id).filter_map(Node::element).any(|a| a.attribute(MC,"Ignorable").is_some_and(|v|
        v.split_whitespace().any(|prefix| a.namespace(prefix) == Some(uri))))
}
fn namespace_available(uri: &str, target: &str) -> bool {
    uri == MC || uri == "http://www.w3.org/XML/1998/namespace" ||
...
fn process_content(doc: &Document, id: usize) -> bool {
    ...
    ancestors(doc,id).filter_map(Node::element).any(|a| a.attribute(MC,"ProcessContent").is_some_and(|v|
        v.split_whitespace().any(|q| q.split_once(':').is_some_and(|(prefix,local)|
            a.namespace(prefix) == Some(name.uri.as_str()) && (local == "*" || local == name.local)))))
}
...
fn mc_validate(doc: &Document, id: usize, issues: &mut Vec<Value>) {
    ...
    for name in ["PreserveElements", "PreserveAttributes", "ProcessContent"] {
        if let Some(value) = e.attribute(MC,name).filter(|v| !v.is_empty()) {
            let valid = !local_ignorable.is_empty() && value.split_whitespace().all(|q|
                q.split_once(':').is_some_and(|(prefix,local)| !local.contains(':') &&
                    defined(prefix).is_some_and(|uri| local_ignorable.contains(&uri))));
            if !valid { issues.push(issue(&format!("mc-{name}"),"markup-compatibility",id,"QNames in this element's Ignorable namespaces",value,&provenance)); }
        }
    }
    ...
    for a in &e.attributes {
        ...
        if xml_space && (wrapper || e.attribute(MC,"ProcessContent").is_some_and(|v| !v.is_empty())) {
            issues.push(issue("mc-xml-attribute","markup-compatibility",id,"no xml:lang/xml:space on MC or ProcessContent host",a.name.local.as_str(),&provenance));
        }
    ...
    if e.name.local == "Choice" {
        ...
        let mut choices = 0; let mut fallback = false; let mut valid = !branches.is_empty();
        for branch in branches {
            let b = doc.node(branch).unwrap().element().unwrap();
            match (b.name.uri.as_str(), b.name.local.as_str()) {
                (MC,"Choice") => { valid &= !fallback; choices += 1; mc_validate(doc,branch,issues); },
                (MC,"Fallback") => { valid &= choices > 0 && !fallback; fallback = true; mc_validate(doc,branch,issues); },
                _ => valid = false,
            }
        }
        if !valid || choices == 0 {
            issues.push(issue("mc-content","markup-compatibility",id,"one or more Choices followed by at most one Fallback","invalid branch sequence",&provenance));
        }
    }
}
...
fn effective(doc: &Document, id: usize, target: &str, skipped: &mut Vec<Value>, issues: &mut Vec<Value>) -> Vec<usize> {
    ...
    for child in children(doc,id) {
        let n = doc.node(child).unwrap(); let e = n.element().unwrap();
        if e.name.uri == MC && e.name.local == "AlternateContent" {
            mc_validate(doc,child,issues);
            let branches = children(doc,child);
            let selected = branches.iter().copied().find(|branch| {
                let b = doc.node(*branch).unwrap().element().unwrap();
                b.name.uri == MC && b.name.local == "Choice" && b.attribute("","Requires").is_some_and(|v|
                    v.split_whitespace().all(|prefix| b.namespace(prefix).is_some_and(|uri| namespace_available(uri,target))))
            }).or_else(|| branches.into_iter().find(|branch| {
                let b = doc.node(*branch).unwrap().element().unwrap(); b.name.uri == MC && b.name.local == "Fallback"
            }));
            ...
            if let Some(branch) = selected {
                result.extend(effective(doc,branch,target,skipped,issues));
            }
        } else if ignorable(doc,child,&e.name.uri) && !schema_index().element_versions.get(&(e.name.uri.as_str(),e.name.local.as_str()))
            .is_some_and(|version| Some(*version) <= rank(target)) {
            if process_content(doc,child) { result.extend(effective(doc,child,target,skipped,issues)); }
            else { skipped.push(json!({"reason":"ignorable-unknown","node":child})); }
...
fn path_nodes(doc: &Document, path: &str, target: &str) -> Vec<usize> {
    ...
    while let Some((id, mut lineage)) = pending.pop() {
        lineage.push(id);
        if lineage.len() >= names.len() && lineage.iter().rev().zip(names.iter().rev())
            .all(|(id,name)| matches(doc.node(*id).unwrap(),name)) { result.push(id); }
...
fn semantic(rule: &'static Value, n: &Node, context: &mut SemanticContext<'_>, gaps: &mut BTreeSet<String>) -> Option<bool> {
    ...
    if let Some(rest) = test.strip_prefix("count(distinct-values(") {
        ...
        if repeated.strip_suffix(')') != Some(query) || !query.chars().all(|c| c.is_ascii_alphanumeric() || ":/_-@.".contains(c)) {
            return None;
        }
        ...
        if names.len() == 1 && resolve(n,None).is_none() || names.len() > 1 &&
            !matches(context.doc.node(context.doc.root).unwrap(),names[0]) { return None; }
        ...
        let duplicates = context.duplicates.entry(test.to_owned()).or_insert_with(|| {
            let mut seen = BTreeSet::new(); let mut duplicates = BTreeSet::new();
    ...
    if let Some(rest) = test.strip_prefix("document(rels)//r:Relationship[@Id = current()/@") {
        ...
        let Some(value) = attribute(n,attr).filter(|v| !v.is_empty()) else { return Some(true); };
        let Some(relationships) = context.relationships else { gaps.insert("relationships".into()); return None; };
    ...
    let Some(doc) = context.dependencies.get(part) else {
        ...
        gaps.insert(format!("dependency:{part}")); return None;
    };
    let values = context.references.entry(test).or_insert_with(|| path_nodes(doc,path,context.target).into_iter()
        .filter_map(|id| attribute(doc.node(id).unwrap(),dest).filter(|v| !v.is_empty()).map(str::to_owned)).collect());
...
pub fn analyze(xml: &Xml, target: &str, dependencies: Option<HashMap<String, PyRef<'_, Xml>>>,
    relationships: Option<HashMap<String, String>>, complete_dependencies: bool) -> PyResult<String> {
    ...
    let mut issues = Vec::new(); let mut gaps = BTreeSet::new(); let mut skipped = Vec::new();
    let mut semantic_context = SemanticContext { doc, dependencies: &deps, relationships: relationships.as_ref(),
        complete_dependencies, target, duplicates: HashMap::new(), references: HashMap::new() };
    let mut active = vec![(doc.root, resolve(doc.node(doc.root)?, None))]; let mut checked = 0; let mut semantic_checked = 0;
    while let Some((index, id)) = active.pop() {
        let n = doc.node(index)?; let e = n.element().unwrap();
        ...
        let Some(id) = id else { gaps.insert(format!("untyped:{{{}}}{}",e.name.uri,e.name.local)); continue; };
        let t = &schema()["types"][&id]; checked += 1;
        ...
        for a in arr(&t["attributes"]) {
            ...
            for error in check_attr(a,value,target,&mut gaps) { issues.push(issue(&format!("attribute-{error}"),"schema",index,a.clone(),json!(value),&t["source"])); }
        }
        for a in &e.attributes {
            if a.name.uri == MC {
            ...
            if !arr(&t["attributes"]).iter().any(|v| expanded(s(&v["QName"])) == (a.name.uri.as_str(),a.name.local.as_str())) {
                if !t["unresolved_base"].is_null() { gaps.insert(format!("undeclared-attribute:{id}:{{{}}}{}",a.name.uri,a.name.local)); }
                else { issues.push(issue("undeclared-attribute","schema",index,"declared attribute",json!([a.name.uri,a.name.local]),&t["source"])); }
            }
        }
        if let Some(p) = schema_index().particles.get(id) {
            let mut local = BTreeSet::new(); let mut budget = 100_000;
            let matching_children: Vec<_> = children.iter().zip(&child_types).map(|(c,t)|
                (t.as_deref().unwrap_or(""),doc.node(*c).unwrap().element().unwrap().name.uri.as_str())).collect();
        ...
        let text: String = n.children.iter().filter_map(|id| match &doc.node(*id).unwrap().kind { NodeKind::Text(t) => Some(t.as_str()), _ => None }).collect();
        ...
        for &(i,rule) in schema_index().semantics.get(&(e.name.uri.as_str(),e.name.local.as_str())).into_iter().flatten() {
            match semantic(rule,n,&mut semantic_context,&mut gaps) {
                Some(valid) => { semantic_checked += 1; if !valid { issues.push(issue(&format!("sdk-schematron-{i}"),"semantic",index,s(&rule["Test"]),"constraint not satisfied",rule)); } }
                None => { gaps.insert(format!("semantic:sdk-schematron-{i}")); }
            }
        }
    }
    Ok(json!({"issues":issues,"target":target,"source":schema()["source"],"coverage":{"schema_nodes_checked":checked,
        "semantic_checks":semantic_checked,"gaps":gaps,"skipped_regions":skipped,"complete":false}}).to_string())
}
