...
fn error(message: impl ToString) -> PyErr { PyValueError::new_err(message.to_string()) }
fn stale() -> PyErr { PyReferenceError::new_err("XML node or document is no longer live") }
...
fn check_value(value: &str) -> PyResult<()> {
...
fn bind(namespaces: &mut Vec<(String, String)>, prefix: &str, uri: &str) -> PyResult<()> {
...
#[derive(Clone, Debug)]
pub(crate) struct Name { pub uri: String, pub local: String, pub prefix: String }
impl Name {
    fn lexical(&self) -> String { if self.prefix.is_empty() { self.local.clone() } else { format!("{}:{}", self.prefix, self.local) } }
    fn resolve(lexical: &str, namespaces: &[(String, String)], attribute: bool) -> PyResult<Self> {
...
#[derive(Clone, Debug)]
pub(crate) struct Attribute { pub name: Name, pub value: String }
#[derive(Clone, Debug)]
...
impl Element {
    ...
    fn edit_name(&mut self, uri: &str, local: &str, prefix: Option<&str>, attribute: bool) -> PyResult<Name> {
        ...
        if !(attribute && prefix.is_empty()) && self.namespace(&prefix) != Some(uri) {
            ...
            bind(&mut self.namespaces, &prefix, uri)?;
        }
        let result = Name { uri: uri.into(), local: local.into(), prefix };
        Name::resolve(&result.lexical(), &self.namespaces, attribute)?;
        Ok(result)
    }
}
#[derive(Clone, Debug)]
...
impl Document {
    pub fn node(&self, id: usize) -> PyResult<&Node> { self.nodes.get(id).and_then(Option::as_ref).ok_or_else(stale) }
    fn node_mut(&mut self, id: usize) -> PyResult<&mut Node> { self.nodes.get_mut(id).and_then(Option::as_mut).ok_or_else(stale) }
    fn element_mut(&mut self, id: usize) -> PyResult<&mut Element> {
    ...
    pub(crate) fn add(&mut self, parent: Option<usize>, kind: NodeKind) -> PyResult<usize> {
    ...
    fn drop_subtree(&mut self, id: usize) {
        ...
        while let Some(id) = stack.pop() { if let Some(node) = self.nodes[id].take() { stack.extend(node.children); } }
    }
    fn position(&self, id: usize) -> PyResult<(Option<usize>, usize)> {
    ...
    fn capacity(&self, additional: usize) -> PyResult<()> {
    ...
    fn inherit_namespaces(&mut self, id: usize) {
        let mut stack = vec![id];
        while let Some(id) = stack.pop() {
    ...
    fn write_node(&self, writer: &mut Writer<Vec<u8>>, id: usize, inherited: &[(String, String)]) -> PyResult<()> {
        let node = self.node(id)?;
        match &node.kind {
            NodeKind::Element(e) => {
...
fn check_pi(target: &str, value: &str) -> PyResult<()> {
    check_value(value)?;
...
fn check_comment(value: &str) -> PyResult<()> {
    check_value(value)?;
...
fn fragment(data: &[u8], namespaces: &[(String, String)]) -> PyResult<Document> {
...
impl Xml {
    pub(crate) fn read(&self) -> PyResult<&Document> { if self.live { Ok(&self.document) } else { Err(stale()) } }
    fn edit<T>(&mut self, operation: impl FnOnce(&mut Document) -> PyResult<T>) -> PyResult<T> {
        self.read()?;
    ...
    fn insert_fragment(doc: &mut Document, parent: Option<usize>, index: usize, data: &[u8], replacing: Option<usize>) -> PyResult<Vec<usize>> {
        if index > doc.sequence(parent)?.len() { return Err(PyIndexError::new_err("XML content index out of range")); }
        ...
        let count = roots.iter().map(|id| source.subtree_size(*id, depth)).collect::<PyResult<Vec<_>>>()?.iter().sum();
        doc.capacity(count)?;
        if parent.is_none() {
...
impl Xml {
    #[new]
    fn new(py: Python<'_>, data: &[u8]) -> PyResult<Self> {
        py.detach(|| {
            let document = parse_bytes(data)?;
            Ok(Self { document, cached_bytes: Some(data.to_vec()), revision: 0, live: true })
        })
    }
    #[getter]
    fn root(&self) -> PyResult<usize> { Ok(self.read()?.root) }
    fn bytes<'py>(&mut self, py: Python<'py>) -> PyResult<Bound<'py, PyBytes>> {
        self.read()?;
        if self.cached_bytes.is_none() { self.cached_bytes = Some(py.detach(|| self.document.serialize())?); }
        Ok(PyBytes::new(py, self.cached_bytes.as_deref().unwrap()))
    }
    fn subtree_bytes<'py>(&self, py: Python<'py>, id: usize) -> PyResult<Bound<'py, PyBytes>> {
        let data = py.detach(|| {
            ...
            let mut writer = Writer::new(Vec::new());
            // Include even an empty default binding: the destination may have a nonempty default.
    ...
    fn attribute(&self, id: usize, uri: &str, local: &str) -> PyResult<Option<String>> {
        let doc = self.read()?;
        let element = doc.node(id)?.element().ok_or_else(|| error("Expected an XML element"))?;
        Ok(element.attributes.iter().find(|a| a.name.uri == uri && a.name.local == local).map(|a| a.value.clone()))
    }
    fn element_ids(&self) -> PyResult<Vec<usize>> { Ok(self.read()?.element_ids()) }
    fn document_children(&self) -> PyResult<Vec<usize>> { Ok(self.read()?.children.clone()) }
    fn children(&self, id: usize) -> PyResult<Vec<usize>> { Ok(self.read()?.node(id)?.children.clone()) }
    fn child_count(&self, id: usize) -> PyResult<usize> { Ok(self.read()?.node(id)?.children.len()) }
    fn element_children(&self, id: usize) -> PyResult<Vec<usize>> {
        let doc = self.read()?;
