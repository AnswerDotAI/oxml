...
def test_package_part_cardinality_and_allowed_relationships(case):
    doc = Document.new()
    related(doc, 'comments', E('w:comments'))
    if case == 'duplicate-comments-parts': related(doc, 'comments', E('w:comments'), '2')
...
def test_header_relationship_type_and_scope(kind, scope):
    doc = Document.new()
    part = doc.package.add_part('/word/target.xml', CT+kind+'+xml', E('w:hdr' if kind == 'header' else 'w:ftr').bytes())
    if scope != 'missing':
        doc.package.add_relationship(doc.main.uri if scope == 'main' else '/', R+kind, part.uri, relationship_id='rIdHeader')
    E('w:sectPr', E('w:headerReference', attrs={'w:type': 'default', 'r:id': 'rIdHeader'})).append_to(next(doc.main.xml.elements(w.Body)))
...
def test_style_id_uniqueness(ids, invalid):
    doc = Document.new()
    part = related(doc, 'styles', E('w:styles', *(E('w:style', attrs={'w:styleId': i}) for i in ids)))
    tree = part.xml
...
def test_duplicate_ids_only_in_selected_compatibility_content(target, invalid):
    doc = Document.new()
    expression = E('w:styles', E('w:style', attrs={'w:styleId': 'Example'}),
        E('mc:AlternateContent',
            E('mc:Choice', E('w:style', attrs={'w:styleId': 'Example'}), attrs={'Requires': 'w14'}),
            E('mc:Fallback', E('w:style', attrs={'w:styleId': 'Other'}))),
...
def test_footnote_reference_uses_related_part(target_id):
    doc = Document.new()
    E('w:p', *(E('w:r', E('w:footnoteReference', attrs={'w:id': '1'})) for _ in range(64))).append_to(next(doc.main.xml.elements(w.Body)))
    if target_id is not None: related(doc, 'footnotes', E('w:footnotes', E('w:footnote', E('w:p'), attrs={'w:id': target_id})))
